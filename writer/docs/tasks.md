...
- [ ] **T2.1** 创建 `writer/ui-category.js` 类目管理器 UI
  - 参考: [ui-design.md](ui-design.md) 第2节 树形结构
  - 树形视图渲染（递归）
  - 展开/折叠交互
  - 选中高亮
  - 添加/编辑/删除按钮（hover 显示）
  - 右键菜单
  - 顶级添加按钮
  - ⚠️ **UI 确认点 UC-02**: 请用户确认树形结构和操作方式
    - 预期: 能看到树形结构，可以展开/折叠，hover 时显示操作按钮，右键有菜单
    - 操作过程: 添加一个大类 → 添加子项 → 展开折叠 → 右键菜单

- [ ] **T2.2** 创建 `writer/ui-detail.js` 详情面板 UI
  - 参考: [ui-design.md](ui-design.md) 第2.3节 详情面板
  - 面包屑导航
  - 名称、描述、属性编辑
  - 版本和修改时间显示
  - 保存按钮
  - 关闭按钮
  - ⚠️ **UI 确认点 UC-03**: 请用户确认详情面板字段和布局
    - 预期: 选中一个类目项后，右侧出现详情面板，显示完整信息，可编辑保存

- [ ] **T2.3** 类目数据 CRUD 业务逻辑
  - 添加: 生成 UUID, version=1, parentId
  - 编辑: version+1, 更新 updatedAt
  - 删除: 递归删除子项，删除关联的 paragraph_bindings
  - 参考: [flow-design.md](flow-design.md) 第8节
...
- [ ] **T2.5** Git commit: "feat(writer): P2 类目管理器"
...
- [ ] **T3.1** 创建 `writer/ui-editor.js` 小说编写区 UI
...
- [ ] **T3.2** 创建 `writer/ui-chat.js` 聊天框 UI
...
- [ ] **T3.3** 创建 `writer/ui-status.js` 当前状态面板 UI
...
- [ ] **T3.6** Git commit: "feat(writer): P3 小说编写界面"

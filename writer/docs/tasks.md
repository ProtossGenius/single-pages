...
> 参考文档: [requirements.md](requirements.md) 第 2、6、7 节, [ui-design.md](ui-design.md) 第 9 节, [architecture.md](architecture.md) 第 4 节

### 本轮开发基础要求

- `writer/` 必须保持为独立页面，不依赖父目录或其他子项目代码
- 如需引入第三方 JS，必须下载到 `writer/vendor/` 本地后再引用
- 每个子任务必须遵循固定工作流：
  1. 实现子功能
  2. 如果是基础单元，添加测试代码到 `writer/test.js` / `writer/test.html`
  3. 运行测试
  4. 确认全部测试通过
  5. 更新本文档对应任务 checkbox
  6. 执行 git commit
- 开发与验证优先使用根目录 HTTPS 服务，访问端口 `8443`
- 修复问题时优先从根因入手，不通过局部 UI 假象掩盖状态或数据一致性问题

### 任务清单

- [x] **T21.1** 复核 P20 遗留问题并更新设计约束
  - 复核 `main` 相对 `origin/main` 未推送的 2 个 commit，确认与 `writer/` 相关的提交范围
  - 将本轮剩余问题映射到模块与根因，补充到计划文档
  - 明确章节目录默认位置、书籍删除联动、最后一段编辑/删除行为的验收标准

- [x] **T21.2** 修复 AI 日志清理仍无效果
  - 校验 `cleanup()` 参数读取、设置保存、事务删除与 UI 输入值传递链路
  - 增加覆盖 UI 输入值与实际删除数量的测试

- [x] **T21.3** 完成最后一段的删除、修改、重新生成闭环
  - 最后一段支持直接删除
  - 点击最后一段进入“更新最后一段”模式时，支持取消并恢复普通生成状态
  - 重新生成与删除后保持段落列表、按钮状态、情节概述状态一致

- [x] **T21.4** 调整左侧栏章节目录优先级并修复重复渲染
  - 将章节目录移动到左侧栏第一个入口
  - 修复章节目录重复显示问题
  - 保证切书、切章、导入数据后目录只渲染一次且状态正确

- [x] **T21.5** 修复删除书籍后的级联清理与界面残留
  - 删除书籍时同步删除其章节、段落、段落绑定及相关状态引用
  - 若删除当前书籍，清空当前章节、正文展示、状态面板与选中项
  - 保证删除后不会继续显示上一本书的章节内容

- [x] **T21.6** 评估并引入本地 UI 辅助库以简化控件控制
  - 仅允许引入本地化第三方库，文件放入 `writer/vendor/`
  - 优先选择体积可控、适合 DOM 查询/事件绑定的库
  - 至少在本轮新增或重构的 UI 代码中落地使用，并补充使用约束

- [x] **T21.7** 补充测试并运行回归
  - 增加书籍删除级联、章节目录去重、最后一段删除/取消、日志清理链路测试
  - 运行 `node writer/run-tests.js`
  - 确认全部测试通过

- [x] **T21.8** 更新文档进度并 git commit
  - 完成后回填本文档 checkbox
  - 执行本轮 git commit
